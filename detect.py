            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        default_label = names[c] if hide_conf else f'{names[c]} {conf:.2f}'
                        label = custom_texts.get(
                            names[c], default_label) if not hide_labels else None
                    ...
                    result_dict = {
                        ...
                        "class": names[int(cls)],
                        "custom_text": custom_texts.get(names[int(cls)], names[int(cls)]),
                        "image_height": im0.shape[0],
                        "image_width": im0.shape[1]
                    }
                    results_json.append(result_dict)
            json_filename = str(save_dir / p.stem) + '.json'
